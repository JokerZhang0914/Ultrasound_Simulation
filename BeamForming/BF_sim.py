...
def main():
    ...
    array_transducer = ArrayTransducer(
        ...
        array_type='curved' if display_mode == '1' else array_type,  # 阵列类型
    ...
    if display_mode == '0':
        # 动态显示模式
        emission_mode = input("请输入发射次数 (single为单次发射，continuous为连续发射): ") or 'single'
        emission_mode = emission_mode if emission_mode in ['single', 'continuous'] else 'single'
